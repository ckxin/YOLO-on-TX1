...
void test_detector(char *datacfg, char *cfgfile, char *weightfile, char *filename, float thresh, float hier_thresh, char *outfile, int fullscreen)
{
    extern int flag;
    extern int flag1;
    extern int grabclick,grabwrite,grabdetect;
    extern int loading;
    extern char labelshowstr[4096];
    char filenamestr[256],detectiontime[256];
    ...
    while(1)
    {
        if(filename)
        {
            ...
            if(grabclick==1)
            {
                while(1)
                {
                    if(grabwrite==1)
                    {
                        strncpy(input,"/home/ubuntu/Qt/Qt-realtime-detection/Grab/grabframe.jpg",256);
                        grabclick=0;
                        grabwrite=0;
                        grabdetect=1;
                        break;
        ...
        if(grabdetect==1)
        {
            ...
            strcat(labelshowstr,filenamestr);
            bzero(filenamestr,sizeof(filenamestr));
        }
        image ima = load_image_color(input,0,0);
        flag=0;
        image sized = letterbox_image(ima, net->w, net->h);
        ...
        time=what_time_is_it_now();
        network_predict(net, X);
        printf("%s: Predicted in %f seconds.\n", "test", what_time_is_it_now()-time);
        if(grabdetect==1)
        {
            sprintf(detectiontime,"%s: Predicted in %f seconds.\n", "test", what_time_is_it_now()-time);
            strcat(labelshowstr,detectiontime);
            bzero(detectiontime,sizeof(detectiontime));
        }
        int nboxes = 0;
        detection *dets = get_network_boxes(net, ima.w, ima.h, thresh, hier_thresh, 0, 1, &nboxes);

        if (nms) do_nms_sort(dets, nboxes, l.classes, nms);
        draw_detections(ima, dets, nboxes, thresh, names, alphabet, l.classes);
...
void run_detector(int argc, char **argv)
{
    ...
    //char *gpu_list = find_char_arg(argc, argv, "-gpus", 1);
